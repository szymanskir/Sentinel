...
def get_hn_comments_jsonpkl():
    hn_comments_jsonpkl_path = join(
        dirname(realpath(__file__)), "hn-stream_comments.json"
    )
...
def test_HackerNewsStreamConnector_create_mention(hn_comments):
    ...
    expected_text = "I always saw these tons of little packages are as a bad \
idea as they make package management (from the user's perspective) harder, they \
do not really provide any space benefit (code is small), if anything they make \
loading slower (i wrote a benchmark some time ago and a binary linked against a \
single dynamic library with tons of exports was faster to load than a binary \
linked against several dynamic libraries with fewer exports - but overall the \
same number of imports) and AFAIK the OS wont keep the entire library in memory \
anyway if it isn't needed (if anything the extra housework needed to all the \
libraries are probably a net negative). And of course these intimidating lists.\
And in practice applications do not use one or two of them, they use several of \
them and those libraries bring in several others, etc like spaghetti (so you have \
Konsole relying on Phonon as mentioned in another reply)."
